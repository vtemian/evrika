      ...
      if(!$_SESSION['rang']){
       ...
       <div class="useri">
		...
		<div class="content">
			...
			<?php
				if($_GET[sterge]==2){
					$query="SELECT * FROM stiri WHERE id='".$_GET[id]."'";
					$result=mysql_query($query);
					if($result){
						$row=mysql_fetch_array($result);
						echo '<div class="useri">
							<form action="admin_stiri_m.php" method="post">
								<label class="mij">Titlu</label>
								<input type="text" name="title" value="'.$row[titlu].'"><br />
						...
						echo '<center><h2>Eroare la selectarea datelor<br /><a href="index.php">Inapoi</a></h2></center>';
					
					
				}else
					if($_GET[sterge]==1){
						$query="DELETE FROM stiri WHERE id='".$_GET[id]."'";
						$result=mysql_query($query);
						if($result){
							echo '<center><h2>Datele au fost sterse cu succes<br /><a href="index.php">Ok</a></h2></center>';
						}else
							echo '<center><h2>Eroare la stergerea datelor<br /><a href="index.php">Inapoi</a></h2></center>';
					}else
						if(isset($_POST[modifica])){
							$query="UPDATE stiri SET articol='".$_POST[content]."',titlu='".$_POST[title]."' WHERE id='".$_POST[id]."'";
							$result=mysql_query($query);
							if($result){
								echo '<center><h2>Datele au fost modifificate cu succes<br /><a href="index.php">Ok</a></h2></center>';
							}else
								echo '<center><h2>Eroare la modificarea datelor<br /><a href="index.php?id='.$_GET[id].'&sterge=0">Inapoi</a></h2></center>';
						}
			?>
