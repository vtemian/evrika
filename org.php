       ...
       <div class="useri">
		...
		<div class="content">
			...
			if($_SESSION[rang]==3){
				if(isset($_POST[trimite])||isset($_POST[modifica])){
					if(trim($_POST[organizator])!=''){
							if(isset($_POST[trimite])){
								if($_POST[part]=="Organizator")
									$m='org';
								else
									$m='partener';
								$query="INSERT INTO diverse (tip,continut,descriere) VALUES('".$m."','".mysql_escape_string($_POST[continut])."','".mysql_escape_string($_POST[organizator])."')";
							}
							else
							...
							$result=mysql_query($query);
							if($result){
								echo "<h2><center>Inserarea datelor s-a realizat cu succes<br /><a href='org.php'>Ok</a></h2></center>";
							}else
								echo "<h2><center>Eroare la inserare<br />".$query."<a href='org.php'>Ok</a></h2></center>";	
						}else
							echo "<h2><center>Trebuie sa alegeti un organizator<br /><a href='org.php'>Ok</a></h2></center>";
						...
						if($_GET[sterge]==1){
							...
							$result=mysql_query($query);
							if($result){
								echo "<h2><center>Datele au fost sterse cu succes<br /><a href='org.php'>Ok</a></h2></center>";
							}else
								echo "<h2><center>Eroare la stergerea datelor<br /><a href='org.php'>Ok</a></h2></center>";
						}else
							if($_GET[sterge]==2){
								...
								if($result){
									...
									echo '<h2>Modific&#259;</h2><br /><hr /><div class="useri">
												<form action="org.php?id='.$_GET[id].'" method="post">
													<label class="mij">Organizator</label>
						...
						$query="SELECT * FROM diverse WHERE tip='org'";
						$result=mysql_query($query);
						if($result){
							echo '<h2>Vizualizare</h2><br /><hr />
								<table>
									<tr><td></td><td></td><td>Organizator</td></tr>
							';
							while($row=mysql_fetch_array($result)){
								echo '	<tr>
											<td><a href="org.php?id='.$row[id].'&sterge=2"><img src="img/edit.png"></a></td>
											<td><a href="org.php?id='.$row[id].'&sterge=1"><img src="img/sterge.png"></a></td>
											<td>'.$row[descriere].'</td>
										</tr>';
							}
						}else
							...
							echo '</table>';
							$query="SELECT * FROM diverse WHERE tip='partener'";
							$result=mysql_query($query);
							if($result){
								echo '<h2>Vizualizare</h2><br /><hr />
									<table>
										<tr><td></td><td></td><td>Partener</td></tr>
								';
								while($row=mysql_fetch_array($result)){
									echo '	<tr>
												<td><a href="org.php?id='.$row[id].'&sterge=2"><img src="img/edit.png"></a></td>
						...
						<form action="org.php" " method="post">
							<input type="radio" name="part" value="Organizator" checked><label>Organizator</label><br>
							<input type="radio" name="part" value="Partener"><label>Partener</label>
				...
				echo "<h1><u>Organizatori</u></h1>";
				$query="SELECT * FROM diverse WHERE tip='org'";
				$result=mysql_query($query);
				if($result){
					while($row=mysql_fetch_array($result)){
						echo '<h3>'.strtoupper($row[descriere]).'</h3>
							<h3>'.strtoupper($row[continut]).'</h3>
						';
					}
				}else
					echo "<h2><center>Eroare la selectarea datelor<br /><a href='org.php'>Ok</a></h2></center>";
				echo "<h1><u>Parteneri</u></h1>";
				$query="SELECT * FROM diverse WHERE tip='partener'";
				$result=mysql_query($query);
				if($result){
					while($row=mysql_fetch_array($result)){
