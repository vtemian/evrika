       ...
       <div class="useri">
		...
		<?php
			...
			if($_GET[comisie]=='1'||$_GET[comisie]=='5'||$_GET[comisie]=='3'){
				if(isset($_POST[submit])){
						...
						if(trim($_POST[prenume])!=''){
							if(trim($_POST[inst])!=''){
								if(trim($_POST[loc])!=''){
									...
									//echo $query;
									if($result){
										echo '<center><h2>Datele au fost introduse cu succes<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>';
									}else
										echo '<center><h2>Eroare la introducerea datelor<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>';
								}else
									echo '<center><h2>Trebuie sa introduceti un o localitate<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>';
							}else
								echo '<center><h2>Trebuie sa introduceti un o institutie<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>';
						}else
							echo '<center><h2>Trebuie sa introduceti un prenume<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>';
					}else
						echo '<center><h2>Trebuie sa introduceti un nume<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>';
					...
					if(isset($_POST[exporta])){
						...
						if($result){
							...
							echo '<center><h2>Datele au fost exportate<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>'; 
						}else
							echo '<center><h2>Eroare la selectarea datelor<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>';
					}else
				...
				echo '<form action="comisie.php?comisie='.$_GET[comisie].'" method="POST"><br /><input type="submit" name="exporta" value="Export" class="button"/><br /><h2>Ad&#259;ugare</h2></ hr><br /><div class="useri">
								<label class="mij">Nume</label>
								<input type="text" name="nume" class="field" /><br /><br />
								<label class="mij">Prenume</label>
								<input type="text" name="prenume" class="field" /><br /><br / >
								<label class="mij">Institu&#355;ia de inv&#259;&#355;&#259;m&#226;nt</label>
								<input type="text" name="inst" class="field" /><br /><br />
								<label class="mij">Localitate</label>
								<input type="text" name="loc" class="field" /><br />
								<label class="mij">Responsabilit&#259;&#355i</label>
								<select name="res">
									<option value="Pre&#351;edinte">Pre&#351;edinte</option>
									<option value="Vicepre&#351;edinte">Vicepre&#351;edinte</option>
									<option value="Vicepre&#351;edinte subcomisie/elaborare subiecte baraj">Vicepre&#351;edinte subcomisie/elaborare subiecte baraj</option>
								...
								<input type="submit" name="submit" value="Inregistreaz&#259;" class="button"><br /><br / ><br />

							</form>
						</div>';
					}	
			}else
				echo '<center><h2>Nu umbla ca poate te arzi<br /><a href="comisie.php?comisie='.$_GET[comisie].'">Ok</a></h2></center>';
			}else{
				if($_GET[comisie]!=4||$_GET[comisie]!=5)
					$query="SELECT * FROM comisie WHERE tip='".mysql_escape_string($_GET[comisie])."' ORDER BY car ASC";
					$result=mysql_query($query);
					if($result){
						if($_GET[comisie]==4)
							echo '  <h2><a href="fisiere/comisie'.$_GET[comisie].'.zip"><img src="img/jos.jpg" width="20" height="20" alt="" /></a>Comisia ';
						else
						...
						while($row=mysql_fetch_array($result)){
							echo '<tr>  <td>'.$row[nume].' '.$row[prenume].'</td>';
										
									
									if($_GET[comisie]==1)echo '<td>'.$row[localitate].'</td>';
							echo '	<td>'.$row[institutie].'</td><td>'.$row[responsabilitate].'</td>
										';
