      ...
      if(!$_SESSION['rang']){
       ...
       <div class="useri">
		...
		<div class="content">
			...
			<?php
				if($_GET[sterge]==2){
					$query="SELECT * FROM program WHERE id='".$_GET[id]."'";
					$result=mysql_query($query);
					if($result){
						$row=mysql_fetch_array($result);
						echo '<div class="useri">
							<form action="admin_prg_m.php" method="post">
								<label class="mij">Data</label>
								<input type="text" name="data" value="'.$row[data].'"><br />
								<textarea name="content">
									';echo $row[articol];echo'
								</textarea><br />
								<center><input type="submit" name="modifica" value="Modific&#259;" class="button"/></center>
					...
					if($_GET[sterge]==1){
						$query="DELETE FROM program WHERE id='".$_GET[id]."'";
						$result=mysql_query($query);
						if($result){
							echo '<center><h2>Datele au fost sterse cu succes<br /><a href="program.php">Ok</a></h2></center>';
						}else
							echo '<center><h2>Eroare la stergerea datelor<br /><a href="program.php">Inapoi</a></h2></center>';
					}else
						if(isset($_POST[modifica])){
							$query="UPDATE program SET articol='".$_POST[content]."',data='".$_POST[data]."' WHERE id='".$_POST[id]."'";
							$result=mysql_query($query);
							if($result){
								echo '<center><h2>Datele au fost modifificate cu succes<br /><a href="program.php">Ok</a></h2></center>';
							}else
								echo '<center><h2>Eroare la modificarea datelor<br /><a href="admin_prg_m.php?id='.$_GET[id].'&sterge=0">Inapoi</a></h2></center>';
						}else
							if($_GET[plus]==1||$_GET[plus]==2){
								
								$query="SELECT ord FROM program WHERE id='".$_GET[id]."'";
								//echo $query;
								$result=mysql_query($query);
								if($result){
									$row=mysql_fetch_array($result);
									if($_GET[plus]==1)
										...
										$ord=$row[ord]+1;
									$query="SELECT id FROM program WHERE ord='".$ord ."'";	
									$res=mysql_query($query);
									if($res){
										$query="UPDATE program SET ord='".$ord."' WHERE id='".$_GET[id]."'";
										...
										if($rez){
											...
											if($_GET[plus]==1)
												...
												$ord--;	
											$query="UPDATE program SET ord='".$ord ."' WHERE id='".$ro[id]."'";
											//echo $query;
											$rs=mysql_query($query);
											if($rs){
												echo '<center><h2>Modificarea s-a realizat cu succes<br /><a href="program.php">Ok</a></h2></center>';
											}else
												echo '<center><h2>Eroare la modificarea datelor<br /><a href="program.php">Ok</a></h2></center>';
										}else
											echo '<center><h2>Eroare la modificarea datelor<br /><a href="program.php">Ok</a></h2></center>';
									}else
										echo '<center><h2>Eroare la modificarea datelor1<br /><a href="program.php">Ok</a></h2></center>';
								}else
									echo '<center><h2>Eroare la selectarea datelor<br /><a href="program.php">Ok</a></h2></center>';
